...
```
atop vi             # NEW nvim + telescope over herm/pi/claude/codex/cursor/…
a-top vi            # same
atop vi list
atop vi <id> get
atop vi <id> set --file PACKET.md   # queued; --wait to block
aos buf catalog|apply|jobs          # rust primitive
...
atop tty -- atop vi # same picker on a local PTY
```

Bare `atop` is still the rust fleet TUI (`atop-tui`). `<CR>` in telescope edits that harness prompt. `<leader>fv` reopens the picker. Never send-keys.
