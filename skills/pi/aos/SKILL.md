...
Live Cos/pi prompt coordination: `aos buf` / `atop vi <id>` (skill `atop-vi`). Never send-keys. Buffer catalog/get/apply is `aos-buf` (`just aos`). Many Cos panes per home; each is its own Hermes session. Do not share a rendered TUI.

## Self-update

Portable binary lives in `bin/aos-buf` (bins `aos` + `aos-buf`). Hosts refresh with `aos update` (GitHub `aos-v*` + SHA256SUMS) or `just aos` (from-source via mise rust / `devbox.json`). Never curl|sh. Pin: `bin/aos-buf/pin.env`. Hermes Cos/CosW stay YAML workers; they do not rsync dirty profile trees. After the first release, vendor to mini/tc2 is `aos update` + ff-only `~/src/profile`.
