...
Multiple Cos panes per `HERMES_HOME` are allowed. Each pane owns its own `tui_gateway` pipe and Hermes session. Extra panes launch `herm --fresh` so they do not resume the live session (that closed the pipe and left synthesizing hung). Steer via `atop vi` buffers, not a shared rendered TUI. `.herm-tui.lock` is a compat pointer, not an exclusive lock.
