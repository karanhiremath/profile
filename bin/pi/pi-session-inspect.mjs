...
for (const file of process.argv.slice(2)) {
  ...
  for (const line of readFileSync(file, 'utf8').split('\n')) {
    ...
    rows.push({
      ...
      tokens: typeof message.usage?.totalTokens === 'number' ? message.usage.totalTokens : null,
      abortSignature: /^This operation was aborted\.?$/i.test(error) ? 'this_operation_was_aborted' : /^Operation aborted\.?$/i.test(error) ? 'operation_aborted' : null,
