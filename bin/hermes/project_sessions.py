...
def doctor() -> tuple[dict[str, Any], bool]:
    ...
    if not shutil.which("tmux"):
        fatal.append("tmux is unavailable on PATH")
    agents = Path(agents_bin())
    if not agents.is_file() or not os.access(agents, os.X_OK):
        fatal.append(f"Hermes agents launcher is missing or not executable: {agents}")
    ...
    if available:
        ...
        if not projects:
            fatal.append("registry directories contain no project YAML files")
    # Per-project issues (cross-host workdirs, incomplete event buses) stay in
    # diagnostics. Only resolver/platform failures fail COSW launch/bootstrap.
    errors = fatal + errors
    result = {
        "ok": not fatal,
