...
class HostThemeTests(unittest.TestCase):
    ...
    def test_steward_linear_theme(self) -> None:
        slin = Path("/tmp/hermes-agents/steward-linear")
        self.assertTrue(mod.is_linear_home(slin))
        self.assertFalse(mod.is_work_home(slin))
        self.assertEqual(mod.profile_class(slin), "linear")
        self.assertEqual(mod.theme_for_home(slin), "nighttide-red")
        self.assertEqual(mod.resolve_profile_class("slin"), "linear")
        self.assertEqual(mod.resolve_profile_class("steward-linear"), "linear")
