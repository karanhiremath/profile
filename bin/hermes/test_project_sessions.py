...
SCRIPT = Path(__file__).resolve().parent / "project_sessions.py"
...
"""
    ...
    def test_cross_host_project_errors_do_not_fail_doctor(self):
        self._registry("gamma", "g-pm", "g-impl", "missing-profile")
        data = yaml.safe_load((self.registry / "gamma.yaml").read_text())
        data["workdir"] = "/no/such/host/path"
        data["tmux"]["workdir"] = "/no/such/host/path"
        (self.registry / "gamma.yaml").write_text(yaml.safe_dump(data, sort_keys=False))
        proc = self.run_script("doctor", check=False)
        self.assertEqual(proc.returncode, 0)
        value = json.loads(proc.stdout)
        self.assertTrue(value["ok"])
        gamma = next(row for row in value["projects"] if row["name"] == "gamma")
        self.assertFalse(gamma["valid"])
        self.assertTrue(any("workdir" in error for error in gamma["errors"]))
