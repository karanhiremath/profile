...
def _render_config(profile: Dict[str, Any]) -> Dict[str, Any]:
    ...
    voice_cfg = profile.get("voice") or {}
    if isinstance(voice_cfg, dict) and voice_cfg:
        rendered_voice = {k: v for k, v in voice_cfg.items() if v is not None}
        if rendered_voice:
            cfg["voice"] = rendered_voice
...
def profile_env_updates(profile: Dict[str, Any]) -> Dict[str, str]:
    """Literal env plus secrets loaded from files. Never prints values."""
    ...
    if isinstance(raw_env, dict):
        for key, value in raw_env.items():
            ...
            if text:
                updates[key] = text
    files = profile.get("env_from_file") or {}
...
def materialize(name: str) -> Path:
    ...
    if uses_voice and "aos-policy voice brief" not in persona:
        persona = (persona + "\n\n" if persona else "") + (
            "Voice: never say the exact sentence \"I'm working on it\" or that "
            "the chief of staff did not reply. Speak a short continuation from "
            "the current a-top / a-top vi snapshot (`aos-policy voice brief`)."
        )
    if persona:
    ...
    env_updates.update(profile_env_updates(profile))
    if uses_voice:
        if base_url:
