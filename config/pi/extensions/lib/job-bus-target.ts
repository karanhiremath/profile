...
/** Cursor Desktop, Cursor-via-pi MCP, and pi-cursor-sdk bridge. */
export function isCursorSession(env: NodeJS.ProcessEnv = process.env): boolean {
	const truthy = (raw: string) => ["1", "true", "yes", "on"].includes(raw);
	for (const key of ["CURSOR_AGENT", "PI_CURSOR_SDK", "PI_CURSOR_BRIDGE"]) {
		const raw = String(env[key] || "").trim().toLowerCase();
		if (truthy(raw)) return true;
	}
	return Boolean(
		String(env.CURSOR_TRACE_ID || "").trim() || String(env.CURSOR_CONVERSATION_ID || "").trim(),
	);
}

export function unscopedBareLaneWarning(project: string, subscriptions: string[]): string {
	if (project !== "unscoped") return "";
	const bare = subscriptions.filter((s) => s && !s.includes(":") && s !== "legacy" && s !== "unscoped");
	if (!bare.length) return "";
	return `warn: project=unscoped; bare lanes [${bare.join(",")}] watch the host-wide unscoped dump. Set PI_JOB_PROJECT or use project:NAME/LANE.`;
}
