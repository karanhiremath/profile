...
function resolveCdevBin(): string {
	const explicit = (process.env.CDEV_SESSION_BIN || "").trim();
	if (explicit) return explicit;
	const candidate = join(homedir(), "src", "karan.hiremath", "scripts", "cdev", "session");
...
function steerCdev(sid: string, text: string): void {
	const bin = resolveCdevBin();
	if (!sid || !bin) return;
	const child = spawn(bin, ["steer", sid, text], {
		stdio: "ignore",
		detached: true,
...
function listText(): string {
	...
	const unscopedWarn = unscopedBareLaneWarning(boundProject, configuredSubs());
	return [
		`inbox: ${boundInbox}`,
