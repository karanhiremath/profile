 ...
 *
 * Generation must go through ctx.modelRegistry.complete so custom providers
 * (Cursor cursor-sdk, including grok-4.6:fast) resolve. pi-ai/compat complete()
 * only knows built-in APIs and throws "No API provider registered for api: cursor-sdk".
 */
...
Do not call tools. Do not read files. Do not search. Output only the handoff prompt text.
...
function extractGeneratedText(response: { stopReason?: string; errorMessage?: string; content: Array<{ type: string; text?: string }> }): string {
	if (response.stopReason === "error") {
		throw new Error(response.errorMessage || "Handoff model returned an error");
	}
	...
	if (!text) {
		throw new Error(`Handoff model returned no text (stopReason: ${response.stopReason ?? "unknown"})`);
	}
